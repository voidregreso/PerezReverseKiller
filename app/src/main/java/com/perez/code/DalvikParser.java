...
import org.jb.dexlib.CodeItem.*;
import org.jb.util.IndentingWriter;
import org.jb.util.LiteralTools;
import org.jb.util.ByteRenderer;
import org.jb.util.LongRenderer;
...
public class DalvikParser {
    ...
    public DalvikParser(CodeItem code) {
        this.code = code;
    }
    ...
    private static void writeInvokeRegister(IndentingWriter writer,
        ...
        writer.write('{');
        if (regCount > 0) {
            writer.write("v" + five.getRegisterD());
        }
        if (regCount > 1) {
            writer.write(",v" + five.getRegisterE());
        }
        if (regCount > 2) {
            writer.write(",v" + five.getRegisterF());
        }
        if (regCount > 3) {
            writer.write(",v" + five.getRegisterG());
        }
        if (regCount > 4) {
            writer.write(",v" + five.getRegisterA());
        }
        writer.write('}');
    }

    private static void writeLabel(IndentingWriter writer, int off)
    throws IOException {
        writer.write(':');
        writer.write(LABEL);
    ...
    private static void writeKeyAndTarget(IndentingWriter writer, int key,
    ...
    private void writeTryItems(IndentingWriter writer) throws IOException {
        ...
        if (tries != null && tries.length > 0) {
            ...
            for (TryItem tryItem : tries) {
                ...
                if (catchAll != -1) {
                    writeCatchBlock(writer, CATCH + ALL, start, end, catchAll);
                } else {
                    EncodedTypeAddrPair[] typeAddrPairs = catchHandler.handlers;
                    for (EncodedTypeAddrPair typeAddrPair : typeAddrPairs) {
                        String exceptionType = typeAddrPair.exceptionType.getTypeDescriptor();
                        int handlerAddress = typeAddrPair.getHandlerAddress();
                        writeCatchBlock(writer, CATCH + exceptionType, start, end, handlerAddress);
                    }
                }
            }
        }
    ...
    private void writeCatchBlock(IndentingWriter writer, String catchType, int start, int end, int handler) throws IOException {
    ...
    private static Pair<List<TryItem>, List<EncodedCatchHandler>> parseCatchs(
        DexFile dexFile, String[] strings, int index,
    ...
    public static boolean searchStringInMethod(
        ClassDataItem.EncodedMethod method, String src) {
    ...
    public static boolean searchMethodInMethod(
        ClassDataItem.EncodedMethod method, String classType, String name,

package com.perez.netdiagnosis.View;
...
public class ProgressWheel extends View {
	...
	private Paint barCirclePaint = new Paint();  
	private Paint rimPaint = new Paint();      
	private Paint textPaint = new Paint();
    private Paint spinRimPaint = new Paint();   
	private Paint spinCirclePaint = new Paint();
	...
	private void setupBounds() {
		...
		arcR = barWidth/2;      
		startArcX = (float) (-circleBounds.width()/2* Math.sin( 2* Math.PI/360*(startDegree+270))+this.getLayoutParams().width/2);   
		...
		if(isSpinning) {
            if(barDegree != -1) {    
                canvas.drawArc(circleBounds, progress - 90, barDegree, false,
					...
					float startX = (float) (-circleBounds.width()/2* Math.sin( 2* Math.PI/360*(progress - 90+270))+this.getLayoutParams().width/2);   
					...
					canvas.drawCircle(x2, y2, tmpR, spinCirclePaint);   
            }else{     
			...
			canvas.drawArc(circleBounds, startDegree, progress, false, barPaint);    
			double t = progress+startDegree+270;
			if(progress != 0) {
				canvas.drawCircle(startArcX, startArcY, arcR, barCirclePaint);    
				...
				canvas.drawCircle(x, y, arcR, barCirclePaint);   
			}
		}

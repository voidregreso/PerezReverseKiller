...
public class CodeEditorActivity extends AppCompatActivity {
    ...
    private DalvikParser dalvikParser;
    ...
    public void init() {
        ...
        if(method.codeItem != null) {
            dalvikParser = new DalvikParser(method.codeItem);
            try {
                StringBuilder sb = new StringBuilder(4 * 1024);
                dalvikParser.dump(new IndentingWriter(sb));
    ...
    private boolean saveCode() {
        try {
            dalvikParser.parse(ClassListActivity.dexFile, codeEdit.getText()
                         .toString());

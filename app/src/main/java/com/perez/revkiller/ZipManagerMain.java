...
public class ZipManagerMain extends AppCompatActivity {
    ...
    public static final String EXTRACTPATH = Environment.getExternalStorageDirectory().getPath() + "/ZipManager";
    public static HashMap<String, byte[]> zipEntries;
    ...
    private Handler mHandler = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            case R.string.replacing:
                ...
                break;
            case ERROR:
                PerezReverseKillerMain.showMessage(ZipManagerMain.this, "", msg.obj.toString());
                break;
            case TOAST:
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        lv.setOnItemClickListener((parent, view, position, id) -> {
            ...
            if(file.toLowerCase().endsWith(".arsc")) {
            ...
            } else if(file.toLowerCase().endsWith(".mp4") || file.toLowerCase().endsWith(".3gp")) {
                Intent intent = new Intent(ZipManagerMain.this, VideoPlayerActivity.class);
                intent.setData(Uri.parse(file.toString()));
                startActivity(intent);
            } else if(file.toLowerCase().endsWith(".mp3") || file.toLowerCase().endsWith(".aac")
                      || file.toLowerCase().endsWith(".ogg") || file.toLowerCase().endsWith(".wma")
                      || file.toLowerCase().endsWith(".wav") || file.toLowerCase().endsWith(".amr")) {
                Intent intent = new Intent(ZipManagerMain.this, AudioPlayer.class);
                ...
                Intent it = new Intent(ZipManagerMain.this, HugeImageViewerActivity.class);
                it.putExtra("IMAGEPATH", file.toString());
                startActivity(it);
            } else if(file.toLowerCase().endsWith(".so")) {
                if(!Features.isValidElf(file.toString())) {
                    Toast.makeText(ZipManagerMain.this, getString(R.string.invalid_elf), Toast.LENGTH_LONG).show();
                    return;
                }
                PELF(new File(file));
            }
    ...
    private void showDialog() {
        PerezReverseKillerMain.prompt(this, getString(R.string.prompt), getString(R.string.is_save),
                (dailog, which) -> {
        ...
        new Thread(() -> {
            ...
            try {
                if(isSigne) {
                    ...
                    temp.deleteOnExit();
                    zip(zipFile, zipEntries, temp);
                    ApkSignerTool.sign(ZipManagerMain.this, temp, out);
                    temp.delete();
                } else {
                    mHandler.sendEmptyMessage(WRITEZIP);
                    File file = new File(out);
                    zip(zipFile, zipEntries, file);
                }
            } catch(Exception e) {
                Message msg = new Message();
                msg.what = ERROR;
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        case R.string.zip_editor_remove:
            PerezReverseKillerMain.prompt(this, getString(R.string.is_remove), name, new DialogInterface.OnClickListener() {
    ...
    private void removeDirectory(String name) {
        Map<String, byte[]> zipEnties = ZipManagerMain.zipEntries;
    ...
    private void textEditArsc(String file) {
        try {
            ...
            Log.v("ZipManager", "File path in ZIP is:" + file.toString());

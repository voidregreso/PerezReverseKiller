...
public class ResAttr extends ResBagValue implements GetResValues {
    ...
    protected String getTypeAsString() {
        ...
        if ((mType & TYPE_REFERENCE) != 0) s.append("|reference");
        if ((mType & TYPE_STRING) != 0) s.append("|string");
        if ((mType & TYPE_INT) != 0) s.append("|integer");
        if ((mType & TYPE_BOOL) != 0) s.append("|boolean");
        if ((mType & TYPE_COLOR) != 0) s.append("|color");
        if ((mType & TYPE_FLOAT) != 0) s.append("|float");
        if ((mType & TYPE_DIMEN) != 0) s.append("|dimension");
        if ((mType & TYPE_FRACTION) != 0) s.append("|fraction");
        return s.length() == 0 ? null : s.substring(1);
    }    

...
char* j2c(JNIEnv *env, jstring jstr) {
    ...
    jboolean isCopy;
    rtn = env->GetStringUTFChars(jstr, &isCopy);
    char *cstr = strdup(rtn); // strdup allocates memory and copies the string.
    env->ReleaseStringUTFChars(jstr, rtn); // Release as soon as we're done.
    return cstr;
}
